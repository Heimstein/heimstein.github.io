...
function DrawTetromino() {
    ...
    for(let i = 0; i < curTetromino.length; i++){
        ...
        if (x < 0 || y < 0 || x >= gBArrayWidth || y >= gBArrayHeight) {
            break;
        }
...
function GetLastSquareX()
{
    ...
    return Math.min(gBArrayWidth - 1, lastX);
}

function ValidateTetromino() {
    let shift = 0;
    for(let i = 0; i < curTetromino.length; i++) {
        if ((curTetromino[i][0] + startX) < 0) {
            shift = Math.max((curTetromino[i][0] + startX) * -1, shift);
        } else if ((curTetromino[i][0] + startX) > gBArrayWidth - 1) {
            shift = Math.min(gBArrayWidth - (curTetromino[i][0] + startX), shift);
        }
    }
    if (shift != 0) {
        console.log(shift);
